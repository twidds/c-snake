...
void arena_init(Arena* arena) {
    ...
    arena->head = malloc(sizeof(MemorySlab));
    arena->head->next = NULL;
    arena->free = &(arena->head->data);
    arena->end = &arena->head->data + sizeof(((MemorySlab*)0)->data);
}
...
void* arena_alloc(Arena* arena, size_t size) {
    ...
    if (arena->free + size_padded > arena->end) { //Allocate new slab
        ...
        arena->free = &last->data;
        arena->end = &last->data + sizeof(((MemorySlab*)0)->data);
    }
...
//"Deallocates" all objects allocated by the arena
