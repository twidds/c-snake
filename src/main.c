...
GameState* new_gamestate() {
    // state->textures = malloc(sizeof(Texture2D) * SPRITESHEET_COUNT);
    // state->textures[SPRITESHEET_FOOD] = LoadTexture("assets/food_spritesheet.bmp");
    // state->textures[SPRITESHEET_SNAKE] = LoadTexture("assets/snake_spritesheet.bmp");
    // state->textures[SPRITESHEET_BACKGROUND] = LoadTexture("assets/backgrounds_spritesheet.bmp");
    ...
    state->screen_size.x = 100;
    state->screen_size.y = 100;
...
//         draw_background(state.textures[SPRITESHEET_BACKGROUND], state.background);
//         EndTextureMode();
...
int main(char** argv, int argc) {
    GameState* state = new_gamestate();
    
    
    InitWindow(state->screen_size.x,state->screen_size.y, "c-snake");
    ...
    // init_state(state);
    // init_snake(&snake);
    // UiElement* menu_items = setup_menu(state->textures);
    // setup_game(state, &snake, &cherry_pos);
    // Scene curScene = SCENE_MENU;


    while (!WindowShouldClose()) {
        if (state->flags[FLAG_SCENECHANGE]) {
            ...
            state->flags[FLAG_SCENECHANGE] = false;
        }
