...
bool spawn_cherry(Snake* snake, iVec2D* cherry_location, iVec2D grid_size) {
...
void init_snake(Snake* snake, iVec2D grid_size) {
...
void setup_gamescreen(SceneState* state) {
    ...
    *gs = (GameState){};
    ...
    gs->background = state->persist_data->selected_background;
    ...
    camera.zoom = screen_size_x / (gs->grid_size.x * SQUARE_PIXEL_WIDTH) ; //base on screen width only right now.

    //init snake
    init_dirqueue(&gs->dir_queue, 2);
    init_snake(gs->snake, state->persist_data->grid_size);
    set_snake(gs->snake, (iVec2D){1,1}, DIR_DOWN, 2, gs->grid_size);
    spawn_cherry(gs->snake, &(gs->cherry_xy), state->persist_data->grid_size);
...
void update_gamescreen(SceneState* scene) {
    ...
    if (gs->flags[GAME_FLAG_PAUSED] || gs->flags[GAME_FLAG_GAMEOVER]) {
        return;
    }
    ...
    //     grow_snake(gs->snake, popf_dirqueue(&(gs->dir_queue)));
    //     gs->flags[GAME_FLAG_ATECHERRY] = false;
    // } else {
    //     move_snake(gs->snake, popf_dirqueue(&(gs->dir_queue)));
    // }

    // if (cherry_collision(gs->snake, gs->cherry_xy)) {
    //     gs->flags[GAME_FLAG_ATECHERRY] = true;
    //     bool spawned = spawn_cherry(gs->snake, &(gs->cherry_xy), gs->grid_size);
...
void draw_gamescreen(SceneState* scene){
    ...
    //     BeginTextureMode(gs->background);
    //     draw_background(state.textures[TEXTURE_BACKGROUND], state.background);
    //     EndTextureMode();
    //     state.flags[GAME_FLAG_BACKGROUNDCHANGE] = false;
    // }
    ...
    // draw_food(cherry_pos, state.textures[SPRITESHEET_FOOD]);
    // draw_snake(snake, state.textures[SPRITESHEET_SNAKE]);
...
// void draw_overlays(SceneState* state, Snake snake, iVec2D cherry_pos){
...
// void run_game(SceneState* state, Snake* snake, iVec2D cherry_pos) {
